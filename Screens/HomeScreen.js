	...
	initiateConversation() {
		...
		Dialogflow_V2.startListening(
			async (result) => {
				...
				this.setState({
					transcript : [ ...this.state.transcript, `You: ${result.queryResult.queryText}` ]
				});
				if (result.queryResult.parameters.foodIngredients) {
					...
					let sentence = `I got a recipe for ${data.title}. You will also need ${allIngredients.join(
						', '
					...
					if (this.state.recipeCounter === 4) {
						...
						this.setState({
							transcript : [ ...this.state.transcript, `Kevin: ${lastRecipeMessage}` ]
						});
					}
					else {
						this.setState({
						...
						let sentence = `I got a new recipe for ${data.title}. You will need ${allIngredients.join(
							', '
					...
					if (this.state.stepCount === this.state.recipeSteps.length) {
						...
						this.setState({
							transcript : [ ...this.state.transcript, `Kevin: ${finishedMessage}` ]
						});
					}
					else {
						let instruction = this.state.recipeSteps[this.state.stepCount];
